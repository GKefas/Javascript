...
const displayMovements = function (acc, sort = false) {
  ...
  // LOOP OVER MOVES TO DISPLAY THEM TO CONTAINER WITH STRING LITERAL
  moves.forEach(function (move, i) {
    const type = move > 0 ? 'deposit' : 'withdrawal';
    const date = new Date(movementsDates[i]);
    const displayDate = formatMovementDate(date, locale);
    const formattedMovement = formatCur(move, locale, currency);

    const html = `
      <div class="movements__row">
        <div class="movements__type-container">
...
const calcDisplayBalance = function (acc) {
  ...
  acc.balance = movements.reduce((acc, move) => acc + move, 0);
...
const calcDisplaySummary = function (acc) {
  ...
  const interest = movements
    ...
    .filter(int => int >= 1) //Exclude interests below 1 euro
...
const createUsernames = function (accs) {
  accs.forEach(function (acc) {
    acc.username = acc.owner
      .toLowerCase()
      .split(' ') //TO ARRAY AND EVERY ELEMENT IS THE WORD OF THE STRING AFTER SPACE
      .map(name => name[0]) // MAP METHOD TO RETURN FIRST LETTER OF EACH WORD
